...
const CourseSideBar = async ({ course, studentId }: CourseSideBarProps) => {
  ...
  const { count: completedSections, sectionsIds: completedSectionsIds } = await getCourseSectionProgressCompletedCount(studentId, publishedSectionsIds);
      ...
      </h1>
      {
        purchase && (
          <div className="mx-3">
            ...
            <p className="text-xs mt-2 text-[#22a094]">%{progressPercentage} <span className="font-bold">completed</span></p>
          </div>
        )
      }
      ...
      <p className="pl-3 text-sm font-bold text-[#9747FF] mt-4">TEMARIO</p>
      {
        publishedSections.map((section) => (
          <Link href={`/courses/${course.id}/section/${section.id}`} key={section.id} className="p-3 rounded-lg hover:bg-[#dfcbfa]">
            ...
            {
              purchase && completedSectionsIds.includes(section.id) ? (
